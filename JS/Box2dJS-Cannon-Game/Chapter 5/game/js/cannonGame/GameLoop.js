...
function GameLoop(_canvasId) {
	...
	// Remove a new game object from the update loop and to the world.
	this.removeGameObject = function(_gameObject){
		gameObjects.splice(gameObjects.indexOf(_gameObject),1);
	}
	
	// Handle a click event for all child objects.
	this.handleClick = function(e){
		
		// Upate all the game objects.
		for (var i = 0; i < gameObjects.length; i++){
			
			if(gameObjects[i].hasOwnProperty('handleClick'))
				gameObjects[i].handleClick(e)
		}
	}
	
	// Return the body given a specific body id.
	...
	// A function used to resolve all collissions in the game.
	function resolveCollissions(){
		...
		for ( contact = GameResources.GameWorld.getWorld().m_contactList; contact != null; contact = contact.m_next )
		{
		    ...
		    var contactA = contact.m_shape1.m_body;
		    var contactB = contact.m_shape2.m_body;
		
			if(contactA == contactB) return;

			// Look up the objects that correspond to the user data of the two contact bodies.
		    var objectA = getObjectById(contactA.userData);
		    var objectB = getObjectById(contactB.userData);

		    // Make sure the objects exist
		    if (!objectA || !objectB) return;
		    if (!objectA.hasOwnProperty('handleCollission') || !objectB.hasOwnProperty('handleCollission') ) return;

		    // Resolve the collission for both objects
		    objectA.handleCollission(objectB);
		    objectB.handleCollission(objectA);
		}
	}
}
