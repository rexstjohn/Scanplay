...
// Create a circle.
-(SPPhysicsObject*)createCircleWithRadius:(float)theRadius atPosition:(CGPoint)point withSkin:(NSString*)aSkinURL isDynamic:(BOOL)dynamic;
...
// Create a Rope.
-(SPPhysicsObject*)createRopeWithAnchor:(SPPhysicsObject*)anAnchor andAttachment:(SPPhysicsObject*)anAttachment withLength:(NSNumber*)length; //rope with two anchor bodies

-(SPPhysicsObject*)createRopeWithAnchor:(SPPhysicsObject*)anAnchor withLength:(NSNumber*)length; //rope with one anchor body

-(SPPhysicsObject*)createRopeWithAnchor:(SPPhysicsObject*)anAnchor withLength:(NSNumber*)length stuckAtPosition:(CGPoint);aPoint //rope with one anchor body with the other end stuck on a point statically in space.

// Create a Gear.
-(SPPhysicsObject*)createGearWithRect:(CGRect)aRect withMotorSpeed:(float)aSpeed; // gear with a motor
